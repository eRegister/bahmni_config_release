...
public class BahmniObsValueCalculator implements ObsValueCalculator {
    ...
    public static enum BmiStatus {
        ...
        SEVERE_MALNUTRITION("Severe Malnutrition < -3SD"),
        MODERATE_MALNUTRITION("Moderate Malnut. >=-3SD<-2SD"),
        NORMAL_ZSCORE("Normal >= -2SD < 1SD"),
        OVERWEIGHT_ZSCORE("Overweight >= 1SD < 2SD"),
        OBESE_ZSCORE("Obese >= 2SD");
    ...
    static def bmiStatus(Double bmi, Integer ageInMonth, String gender) {
        BMIChart bmiChart = readCSV(OpenmrsUtil.getApplicationDataDirectory() + "obscalculator/BMI_chart_zscores.csv");
    ...
    static class BMIChartLine {
        ...
        public Double third;            // -3SD
        public Double fifteenth;        // -2SD
        public Double eightyFifth;      // 1SD
        public Double ninetySeventh;    // 2SD
        ...
        public BmiStatus getStatus(Double bmi) {
            if(bmi < third) {
                return BmiStatus.SEVERE_MALNUTRITION
            } else if(bmi < fifteenth) {
                return BmiStatus.MODERATE_MALNUTRITION
            } else if(bmi < eightyFifth) {
                return BmiStatus.NORMAL
            } else if(bmi < ninetySeventh) {
                return BmiStatus.OVERWEIGHT_ZSCORE
            } else {
                return BmiStatus.OBESE_ZSCORE
            }
        }
    }
