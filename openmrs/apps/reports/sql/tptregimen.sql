SELECT
	Patient_Identifier, Patient_Name, Age, Gender, age_group, Program_Status, regimen_name, sort_order
FROM(
        (
            SELECT Id, patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated on TB' AS 'Program_Status', sort_order
	FROM
		...
		from obs o
			...
			INNER JOIN patient ON o.person_id = patient.patient_id
				AND (o.concept_id = 2237 AND DATE(o.value_datetime) BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE)) -- include clients initiated in #startDate#' and #endDate#'
				AND patient.voided = 0 AND o.voided = 0
				AND o.person_id not in ( -- exclude TB transfer ins
				    select distinct os.person_id
				from obs os
				where
					      os.concept_id = 3772 AND os.value_coded = 2095
					AND (os.obs_datetime BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE))
						 )
				AND o.person_id not in -- exclude patients who passed on
						(
						    select distinct person_id
				from person
				where death_date < CAST('#endDate#' AS DATE) and dead = 1
						)
				AND o.person_id not in -- exclude transfer outs
						(
							select distinct os.person_id
				from obs os
...
-- left with picking specifically TPT Clients out of these TB Clients
...
select distinct patient.patient_id AS Id,
	...
	from obs o
		-- CLIENTS SEEN FOR TB
		INNER JOIN patient ON o.person_id = patient.patient_id
			AND (o.concept_id = 4929 AND o.value_coded = 2928 OR o.value_coded = 4928) -- picking the ones filled in TPT / ITP Form
		...
		INNER JOIN patient_identifier ON patient_identifier.patient_id = person.person_id AND patient_identifier.identifier_type = 3
		INNER JOIN reporting_age_group AS observed_age_group ON
									  CAST('#endDate#' AS DATE) BETWEEN (DATE_ADD(DATE_ADD(person.birthdate, INTERVAL observed_age_group.min_years YEAR)
, INTERVAL observed_age_group.min_days DAY))
									  AND
(DATE_ADD
(DATE_ADD
(person.birthdate, INTERVAL observed_age_group.max_years YEAR), INTERVAL observed_age_group.max_days DAY))
           WHERE observed_age_group.report_group_name = 'TB_ages'

) AS Clients_Seen

WHERE Clients_Seen.Id not in
( -- deduplicating clients seen from clients intiated in the month
		select distinct patient.patient_id AS Id
from obs o
	-- CLIENTS NEWLY INITIATED ON ART
	INNER JOIN patient ON o.person_id = patient.patient_id
...
AND Clients_Seen.Id not in
-- excluding clients who passed on
(
			select distinct person_id
from person
...
UNION
-- yet work on previous months
-- and also to wrap head around where to use buttons from Obs or Orders from Medications tab
...
(select distinct patient.patient_id AS Id,
        ...
        from obs o
				-- CAME IN PREVIOUS 1 MONTH AND WAS GIVEN (2, 3, 4, 5, 6 MONTHS SUPPLY OF DRUGS)
                 INNER JOIN patient ON o.person_id = patient.patient_id 
...
(select distinct patient.patient_id AS Id,
                ...
                from obs o
				-- CAME IN PREVIOUS 2 MONTHS AND WAS GIVEN (3, 4, 5, 6 MONTHS SUPPLY OF DRUGS)
                 INNER JOIN patient ON o.person_id = patient.patient_id 
...
(select distinct patient.patient_id AS Id,
                ...
                from obs o
				-- CAME IN PREVIOUS 4 MONTHS AND WAS GIVEN (5, 6 MONTHS SUPPLY OF DRUGS)
                 INNER JOIN patient ON o.person_id = patient.patient_id 
...
(select distinct patient.patient_id AS Id,
                ...
                from obs o
				-- CAME IN PREVIOUS 5 MONTHS AND WAS GIVEN (6 MONTHS SUPPLY OF DRUGS)
                 INNER JOIN patient ON o.person_id = patient.patient_id 
