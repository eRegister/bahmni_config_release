...
(
	(SELECT Id,patientIdentifier AS "Patient_Identifier",UniqueNumber AS "ART_Unique_Number", patientName AS "Patient_Name", Age,DOB, Gender, age_group, 'Initiated' AS 'Program_Status'
	FROM
                ...
                from obs o
						-- CLIENTS NEWLY INITIATED ON ART
						 ...
						 INNER JOIN patient_identifier pid ON pid.patient_id = person.person_id and pid.identifier_type = 3 AND pid.preferred=1
						 INNER JOIN patient_identifier pat ON pid.patient_id = pat.patient_id and pat.identifier_type = 5
...
(SELECT Id,patientIdentifier AS "Patient_Identifier",UniqueNumber AS "ART_Unique_Number", patientName AS "Patient_Name", Age,DOB, Gender, age_group, 'Seen' AS 'Program_Status'
FROM (
...
(SELECT Id,patientIdentifier AS "Patient Identifier",UniqueNumber AS "ART_Unique_Number", patientName AS "Patient Name", Age, Gender, age_group, 'Seen_Prev_Months' AS 'Program_Status', sort_order
FROM (
(select distinct patient.patient_id AS Id,
                                   pid.identifier AS patientIdentifier,
									   pat.identifier AS UniqueNumber,
...
from obs a
inner join 
		(select o.person_id,cast(max(obs_datetime) as date) maxdate 
		from obs o 
