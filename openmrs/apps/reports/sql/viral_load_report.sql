						...
						from obs o
								...
								--  CLients with with viral load results
								 AND o.person_id in (
									...
									from obs os
									-- where os.concept_id = 4268 AND DATEDIFF(CAST('#endDate' AS DATE), os.value_datetime INTERVAL -3 MONTH)
    ...
    (SELECT DISTINCT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Pending' AS 'VL_Results_Status', sort_order, Date_Specimen_Collected, Duration_Results_Pending
		FROM
						(select distinct patient.patient_id AS Id,
											   ...
											   observed_age_group.name AS age_group,
                                               o.value_datetime AS Date_Specimen_Collected,
											   concat(floor(datediff(CAST('#endDate#' AS DATE), o.value_datetime)), ' ', 'days') AS Duration_Results_Pending,
											   observed_age_group.sort_order AS sort_order
						...
						from obs o
								...
								--  clients without Viral load results
						           AND o.person_id not in (
									select distinct os.person_id 
									from obs os
									where os.concept_id = 4268 AND  MONTH(o.value_datetime) IN (MONTH(DATE_ADD(CAST('#endDate#' AS DATE), INTERVAL -3 MONTH)), MONTH(DATE_ADD(CAST('#endDate#' AS DATE), INTERVAL -2 MONTH)), MONTH(DATE_ADD(CAST('#endDate#' AS DATE), INTERVAL -1 MONTH)), MONTH(DATE_ADD(CAST('#endDate#' AS DATE), INTERVAL -0 MONTH)))								
									AND patient.voided = 0 AND o.voided = 0
								 )
