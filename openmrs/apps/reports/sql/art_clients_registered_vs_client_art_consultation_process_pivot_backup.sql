SELECT DISTINCT
	   ...
	   round((SUM(IF(ART_Consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0))/SUM(1))*100) AS '% with ART Consultations',
	   round((SUM(IF(ART_Consultations_vs_Registrations_Agg.status = 'Not Consulted', 1, 0))/SUM(1))*100) AS '% without ART Consultations'	   
FROM
(
		...
		(
			...
			FROM
					...
					WHERE v.date_started >= CAST('#startDate#' AS DATE) and v.date_started <= CAST('#endDate#' AS DATE)) reg
