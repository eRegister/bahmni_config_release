SELECT HTS_TOTALS_COLS_ROWS.AgeGroup
        ...
        , HTS_TOTALS_COLS_ROWS.Self_Test_Positives
        , HTS_TOTALS_COLS_ROWS.Self_Test_Negatives
...
FROM (
			...
			(SELECT HTS_STATUS_DRVD_ROWS.age_group AS 'AgeGroup'
					, HTS_STATUS_DRVD_ROWS.Gender
						...
						, IF(HTS_STATUS_DRVD_ROWS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_ROWS.HIV_Testing_Initiation = 'PITC'
                            ...
                            , IF(HTS_STATUS_DRVD_ROWS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_ROWS.HIV_Testing_Initiation = 'Self_Test_Positives'
							AND HTS_STATUS_DRVD_ROWS.HIV_Status = 'Positive', 1, 0))) AS Self_Test_Positives
                             , IF(HTS_STATUS_DRVD_ROWS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_ROWS.HIV_Testing_Initiation = 'Self_Test_Negatives'
							AND HTS_STATUS_DRVD_ROWS.HIV_Status = 'Negative' , 1, 0))) AS Self_Test_Negatives
			...
			FROM (SELECT Id,Patient_Identifier, Patient_Name, Age, Gender, age_group, HIV_Testing_Initiation , Testing_History , HIV_Status, sort_order
FROM (
    ...
    FROM
            ...
            from obs o
                -- HTS CLIENTS WITH HIV STATUS BY SEX AND AGE
                 ...
                 AND o.obs_datetime BETWEEN CAST('2020-10-01' AS DATE) AND CAST('2021-02-28' AS DATE)
                 ...
                 AND o.person_id in (
                  ...
                  AND os.obs_datetime BETWEEN CAST('2020-10-01' AS DATE) AND CAST('2021-02-28' AS DATE)
                  AND patient.voided = 0 AND o.voided = 0
                 )
    ...
    (SELECT Id,patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Self-test' AS 'HIV_Testing_Initiation'
                     , 'SELF TEST' AS 'Testing_History'     , HIV_Status, sort_order
    ...
    FROM
            (select distinct patient.patient_id AS Id,
                         ...
                         observed_age_group.sort_order AS sort_order
            from obs o
                -- HTS SELF TEST CLIENTS WITH HIV STATUS BY SEX AND AGE
                 INNER JOIN patient ON o.person_id = patient.patient_id 
...
ORDER BY HTS_Status_Detailed.HIV_Testing_Initiation
	...
	UNION ALL
            ...
            , IF(HTS_STATUS_DRVD_COLS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_COLS.HIV_Testing_Initiation = 'Self_Test_Positives'
							AND HTS_STATUS_DRVD_COLS.HIV_Status = 'Positive', 1, 0))) AS Self_Test_Positives
                             , IF(HTS_STATUS_DRVD_COLS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_COLS.HIV_Testing_Initiation = 'Self_Test_Negatives'
							AND HTS_STATUS_DRVD_COLS.HIV_Status = 'Negative' , 1, 0))) AS Self_Test_Negatives
						, IF(HTS_STATUS_DRVD_COLS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_COLS.HIV_Testing_Initiation = 'PITC', 1, 0))) as 'Total'
						, 99 AS sort_order
			FROM (SELECT Id,Patient_Identifier, Patient_Name, Age, Gender, age_group, HIV_Testing_Initiation , Testing_History , HIV_Status
FROM (
