...
FROM (
			...
			FROM (
					...
					(SELECT Id, patientIdentifier AS "Patient Identifier", patientName AS "Patient Name", Age, Gender, age_group, HIV_Status, 'first' AS 'Visit', sort_order
					FROM
					...
					FROM
							 ...
							 -- ANC FIRST VISIT
								 ...
								 AND o.person_id in (
									select distinct os.person_id
									from obs os
									where os.concept_id = 2165 and os.value_coded = 1738
									AND os.obs_datetime BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE)
									AND patient.voided = 0 AND o.voided = 0
								 )
	 ...
	 UNION ALL
			...
			(SELECT 'Total' AS 'AgeGroup'
						...
						, IF(HTS_STATUS_DRVD_COLS.Id IS NULL, 0, SUM(IF(HTS_STATUS_DRVD_COLS.Visit = 'first' AND HTS_STATUS_DRVD_COLS.HIV_Status = 'Positive', 1, 0))) AS Negatives
			...
			FROM (
					...
					(SELECT Id, patientIdentifier AS "Patient Identifier", patientName AS "Patient Name", HIV_Status, 'first' AS 'Visit'
