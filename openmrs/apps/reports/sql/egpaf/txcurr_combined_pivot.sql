							...
							(
									...
									where o.person_id in (
											...
											(
												select oss.person_id, SUBSTRING(MAX(CONCAT(oss.value_datetime, oss.obs_id)), 20) AS observation_id, max(oss.value_datetime) as latest_followup_obs
												from obs oss
												where oss.voided=0 and
													oss.concept_id=3752 and datediff(oss.value_datetime, CAST('#startDate#' AS DATE)) > 0 and oss.obs_datetime < CAST('#startDate#' AS DATE)
													and oss.obs_datetime > DATE_ADD(CAST('#startDate#' AS DATE), INTERVAL -7 MONTH)
									...
									and o.person_id not in (
											...
											(
												...
												where os.concept_id=2266
												group by os.person_id
												having latest_transferout < CAST('#startDate#' AS DATE)
											) as TOUTS
									)
							)							 
								 INNER JOIN person ON person.person_id = patient.patient_id AND person.voided = 0
