SELECT Total_Aggregated_TxCurr.AgeGroup
		, Total_Aggregated_TxCurr.Males
		, Total_Aggregated_TxCurr.Females
...
(
	(SELECT TXCURR_DETAILS.age_group AS 'AgeGroup'
			, IF(TXCURR_DETAILS.Id IS NULL, 0, SUM(IF(TXCURR_DETAILS.Gender = 'M', 1, 0))) AS Males
			, IF(TXCURR_DETAILS.Id IS NULL, 0, SUM(IF(TXCURR_DETAILS.Gender = 'F', 1, 0))) AS Females
...
(SELECT 'Total' AS AgeGroup
		, IF(Totals.Id IS NULL, 0, SUM(IF(Totals.Gender = 'M', 1, 0))) AS 'Males'
		, IF(Totals.Id IS NULL, 0, SUM(IF(Totals.Gender = 'F', 1, 0))) AS 'Females'
