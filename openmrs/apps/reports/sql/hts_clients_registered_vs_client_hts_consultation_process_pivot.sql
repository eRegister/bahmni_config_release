SELECT DISTINCT
	   ART_Consultations_vs_Registrations_Agg.location_name AS Location,
	   SUM(1) AS 'Total Visits Registered',	   
	   SUM(IF(ART_Consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0)) AS Consulted,
	   round((SUM(IF(ART_Consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0))/SUM(1))*100) AS '% with HTS Consultations',
	   round((SUM(IF(ART_Consultations_vs_Registrations_Agg.status = 'Not Consulted', 1, 0))/SUM(1))*100) AS '% without HTS Consultations'	   
FROM
(
		...
		(
			...
			FROM
					...
					WHERE en.encounter_datetime >= CAST('#startDate#' AS DATE) and en.encounter_datetime <= CAST('#endDate#' AS DATE) and v.visit_type_id = 15) reg
					...
					FROM visit v
							...
							JOIN obs o on o.encounter_id=en.encounter_id and o.concept_id=2160
							JOIN location l on v.location_id = l.location_id and l.retired=0 and l.name like '%ART%'
			...
			FROM
					...
					WHERE v.date_started >= CAST('#startDate#' AS DATE) and v.date_started <= CAST('#endDate#' AS DATE) and v.visit_type_id = 15) reg
		...
		) AS RegisteredWithARTConsultations
		-- GROUP BY RegisteredWithARTConsultations.id


) AS ART_Consultations_vs_Registrations_Agg

GROUP BY ART_Consultations_vs_Registrations_Agg.location_name
